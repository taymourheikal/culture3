...
Brain evaluation is selected per simulation tick, but current automatic selection stays on sync evaluation because browser benchmarks showed sync faster than both object-payload and compact-payload nested brain Workers through 1000 fixed-pop agents and through churn scenarios. The nested Worker pool remains available for diagnostics, parity tests, and benchmark scripts; it is not a production speed path right now. If the threshold is enabled later, mode selection applies on the next tick selection when population crosses the threshold.

The brain-evaluation pool reports its effective mode in diagnostic runs. Repeated worker failures or timeouts temporarily disable parallel evaluation and fall back to sync immediately. Pause and stop update run state immediately but take effect at the next tick boundary: an in-flight tick may finish, and no new ticks start afterward. Reset and new-session paths still invalidate in-flight brain work.
