import { SPAWNER_INPUT_METADATA_TEXT } from "../inputMetadataText";
...
export function SineHelpContent() {
  return (
    <section className="sine-help-content">
        ...
        <article id="runs" className="sine-help-panel sine-help-panel-wide">
          <div className="sine-help-panel-title">Runs And Headless Experiments</div>
          <p>
          ...
          <div className="sine-help-columns">
            <div>
              <p className="sine-help-section-label">What gets saved</p>
              <ul>
            ...
            <div>
              <p className="sine-help-section-label">What the analysis view means</p>
              <ul>
                <li>The leaderboard is a sortable table of eligible agents, not a seed-bank selector.</li>
                <li>Trade breakdowns use eligible persisted trade rows and show final resolved-trade coverage for context.</li>
                <li>Opening an agent shows its lifecycle, snapshots, RNN architecture, and saved trade ledger.</li>
              </ul>
            </div>
          </div>
          <p>
            <strong>Minimum resolved trades controls persistence eligibility.</strong> Agents below that threshold may have
            existed in the run, but their buffered trade and snapshot detail is not promoted into the analysis tables unless
            they qualify.
          </p>
        </article>
        ...
        <article id="agents" className="sine-help-panel sine-help-panel-wide">
          ...
          <p>
            ...
            inherit and mutate the tick windows used to calculate its relative changes, volume context, RSI, trend, roughness,
            cycle, and pending density inputs. Population room is a fixed context input where 1 means open/empty and 0 means
            the population cap is full.
          </p>
          <div className="sine-help-columns">
            <div>
        ...
        <article id="io" className="sine-help-panel sine-help-panel-wide">
          ...
          <div className="sine-help-columns">
            <div>
              ...
              <ul className="sine-input-metadata-list">
                {SPAWNER_INPUT_METADATA_TEXT.map((input) => (
                  <li key={input.index}>
          ...
          <p>
            <strong>Population room is the twenty-second input.</strong> It tells the brain how much space remains under the
        ...
        <article id="seed-bank" className="sine-help-panel sine-help-panel-wide">
          <div className="sine-help-panel-title">Seed Bank Direction</div>
          <p>
            <strong>The seed bank is not implemented as a simple top-agent list.</strong> A plain payoff leaderboard can be
            dominated by one lineage, one behavior, or one market condition. The intended design is to save agents based on
            where they performed well in market-regime space.
          </p>
          <div className="sine-help-columns">
            <div>
              <p className="sine-help-section-label">Market-regime embedding</p>
              <ul>
                <li>Segment a headless run into market windows.</li>
                <li>Compute market-only features such as volatility, trend, roughness, drawdown, range, and transition score.</li>
                <li>Cluster or embed those windows into regime regions without using agent payoff or agent behavior.</li>
              </ul>
            </div>
            <div>
              <p className="sine-help-section-label">Candidate selection goal</p>
              <ul>
                <li>Find agents that perform well in specific market regions.</li>
                <li>Find generalists that perform acceptably across multiple regions.</li>
                <li>Avoid filling the bank with many agents that only cover the same market pocket.</li>
              </ul>
            </div>
          </div>
          <p>
            <strong>Lookahead bias matters.</strong> For live reseeding, a market-regime label must use only information that
            would have been available at the decision time. Post-run descriptive labels can use richer windows, but those
            should be treated as research labels rather than live selection signals.
          </p>
        </article>
          ...
          <div className="sine-help-columns">
            <div>
              ...
              <p>
                ...
                fallback. Current automatic selection stays on sync evaluation because browser benchmarks showed sync was
                faster than both object-payload and compact-payload worker paths through 1000 fixed-pop agents and churn
                scenarios. Compact worker mode is retained for diagnostics because it preserves optimized trace materialization
                after browser-worker transfer, but it is not the measured default runtime path.
              </p>
            </div>
            <div>
